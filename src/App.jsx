...
// ─── Bindhify Wordmark ────────────────────────────────────────────────────────
// The two dots on the i's are styled as bindi dots for brand identity
...
function BindhiWordmark({ size = 62, style: extra = {} }) {
  ...
  return (
    <span style={base}>B<BindiI />ndh<BindiI />fy</span>
  )
}
...
const css = {
  page: {
    minHeight: '100vh',
    background: 'linear-gradient(160deg, #FDF8F2 0%, #F5EDE0 100%)',
    display: 'flex',
  ...
  entryCard: {
    ...
    boxShadow: '0 6px 24px rgba(28,20,16,0.07)',
    cursor: 'pointer',
  ...
  uploadBtn: {
    ...
    boxShadow: '0 4px 20px rgba(232,97,10,0.35)',
  },
  privacyNote: {
    fontSize: '11px',
    color: '#BBB0A8',
  ...
  result: {
    ...
    borderRadius: '24px',
    overflow: 'hidden',
    boxShadow: '0 16px 56px rgba(28,20,16,0.16), 0 2px 8px rgba(28,20,16,0.08)',
    animation: 'fadeUp 0.4s ease both',
  },
  header: {
    display: 'flex',
  ...
  genderBtn: (active) => ({
    ...
    fontFamily: "'DM Sans', sans-serif",
    borderRadius: '8px',
    border: active ? '2px solid #E8610A' : '1px solid #DDD5C8',
    background: active ? '#FEF3EB' : '#FAFAFA',
  ...
  noteBox: {
    marginTop: '12px',
    padding: '12px 16px',
    background: 'linear-gradient(135deg, #FEF3EB, #FDF0E8)',
    borderRadius: '12px',
  ...
  downloadBtn: {
    ...
    cursor: 'pointer',
    border: 'none',
    boxShadow: '0 4px 16px rgba(232,97,10,0.3)',
...
function ApiKeySetup({ onSave }) {
  ...
  return (
    <div style={css.apiSetup}>
      <BindhiWordmark size={32} style={{ marginBottom: '16px' }} />
      <p style={{ fontSize: '14px', color: '#7A6E65', lineHeight: 1.6 }}>
        To run the prototype locally, enter your Anthropic API key. It is stored only in your browser's session memory and never sent anywhere except the Anthropic API.
      </p>
      <input
        style={css.apiInput}
        type="password"
      ...
      />
      <button style={css.apiSaveBtn} onClick={() => onSave(key)} disabled={!key.startsWith('sk-')}>
        Save & continue
      </button>
      <p style={{ fontSize: '12px', color: '#AAA09A' }}>
        Get your key at console.anthropic.com · For the final product, this would live server-side.
      </p>
    </div>
  )
}
...
// Bindi style dot previews — colored circles representing the palette
const PREVIEW_DOTS = [
...
function Landing({ onUpload, onCamera }) {
  ...
  return (
    <div style={css.landing}>
      ...
      {/* Bindi dot palette — visual preview of what the app does */}
      <div style={css.bindiDots}>
        {PREVIEW_DOTS.map((d, i) => (
          <div key={i} style={{
            ...
            borderRadius: '50%',
            background: d.color,
            boxShadow: `0 0 ${d.size}px ${d.glow}, 0 2px 6px rgba(0,0,0,0.12)`,
            flexShrink: 0,
          }} />
        ))}
      </div>
      ...
      {/* Cultural context */}
      <p style={css.heroline}>
        Explore bindi and tilak styles worn across South Asia for over 5,000 years — now try them on, find your mark.
      </p>
...
export default function App() {
  const [phase, setPhase] = useState('landing') // landing | processing | result | error
  ...
  const handleUpload = useCallback(async (e) => {
    ...
    try {
      ...
      // Run MediaPipe landmark detection + Claude gender detection in parallel
      ...
      // Use detected gender to pick the right catalogue
  ...
  async function handleCapture() {
    ...
    if (!captured) return
    setImageUrl(captured.dataUrl)
    setDetection(captured.detection)
    setPhase('result')
  }
    ...
    <div style={css.page}>
      ...
      {phase === 'error' && (
        <div style={{ width: '100%', maxWidth: '480px', padding: '24px', animation: 'fadeUp 0.4s ease both' }}>
          ...
          <button style={{ ...css.uploadBtn, marginTop: '12px', display: 'block' }} onClick={reset}>
            Try another photo
          </button>
        </div>
      )}
        ...
        <div style={css.result}>
          ...
          <div style={css.panel}>
            ...
            <div style={css.genderToggle}>
              <button style={css.genderBtn(catalogue === BINDIS)} onClick={() => { setCatalogue(BINDIS); selectStyle(BINDIS[0]) }}>
                Bindi (female)
              </button>
              <button style={css.genderBtn(catalogue === TILAKS)} onClick={() => { setCatalogue(TILAKS); selectStyle(TILAKS[0]) }}>
                Tilak (male)
              </button>
            </div>
            <div style={css.sectionLabel}>Choose a style</div>
            <StyleCarousel styles={catalogue} selected={selectedStyle} onSelect={s => selectStyle(s)} />
            ...
            <div style={css.controls}>
              ...
              <input type="range" min="0.4" max="2.0" step="0.05" value={size}
                style={css.slider} onChange={e => setSize(parseFloat(e.target.value))} />
            </div>
            <div style={css.controls}>
              <span style={css.sliderLabel}>Position</span>
              <input type="range" min="-30" max="30" step="1" value={nudge}
                style={css.slider} onChange={e => setNudge(parseInt(e.target.value))} />
            </div>
            {selectedStyle && (
              <div style={css.noteBox}>
                <span style={{ fontStyle: 'normal', fontWeight: 500, color: '#E8610A' }}>
                  {selectedStyle.name} —{' '}
                </span>
                {selectedStyle.note}
              </div>
      ...
      {phase === 'result' && (
        <div style={css.result}>
          ...
          <div style={css.panel}>
            ...
            {/* Style carousel */}
            <div style={css.sectionLabel}>Choose a style</div>
            <StyleCarousel styles={catalogue} selected={selectedStyle} onSelect={s => { selectStyle(s); setShowInfo(false) }} />
          ...
          <div style={css.actions}>
            <button style={css.downloadBtn} onClick={handleDownload}
              onMouseEnter={e => e.target.style.background = '#D05508'}
              onMouseLeave={e => e.target.style.background = '#E8610A'}>
              Download
            </button>
